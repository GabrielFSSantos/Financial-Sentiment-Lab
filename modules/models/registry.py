...
from modules.models.base import BaseSentimentModel, ModelPrediction
from modules.models.common import ASSET_FETCH_HINT, to_serializable
...
class ModelRegistry:
    ...
    def from_resolved_configuration(
        ...
        models = getattr(configuration, "models", None)
        if models is None:
            raise ModelRegistryError(
                "A configuração resolvida não possui modelos selecionados."
            )
        return cls(models)
    ...
    def resolve_adapter_class(
        ...
        if not issubclass(
            ...
            raise ModelAdapterContractError(
                ...
                "herdar de modules.models.base.BaseSentimentModel."
            )
...
def create_model_registry(
    configuration: Any | Iterable[ModelConfiguration],
) -> ModelRegistry:
    """Cria um registry a partir da configuração resolvida ou de modelos."""

    models = getattr(configuration, "models", None)
    if models is not None:
        return ModelRegistry.from_resolved_configuration(configuration)
