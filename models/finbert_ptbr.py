...
"""Adaptador local do FinBERT-PT-BR.

O adaptador valida os arquivos do modelo, carrega tokenizer e pesos sem
acesso à internet, executa inferência em lotes e converte as saídas para
``ModelPrediction``.

O ``config.json`` público do FinBERT-PT-BR declara
``problem_type="multi_label_classification"``. A pipeline do projeto,
porém, trabalha com três classes mutuamente exclusivas e exige
probabilidades que somem 1. Por isso, a função de probabilidade é
explícita e registrada nos metadados:

- ``softmax``: padrão do projeto;
- ``sigmoid``: preserva os escores sigmoid brutos e os normaliza por L1
  para o contrato probabilístico da pipeline;
- ``model_config``: escolhe sigmoid ou softmax a partir do config do
  modelo.

Nenhuma configuração é alterada silenciosamente dentro do objeto do
Transformers, e os escores brutos permanecem disponíveis em ``extra``.
"""
...
class FinBertPtBrModel(BaseSentimentModel):
    """Adaptador do modelo ``lucas-leme/FinBERT-PT-BR``."""
    ...
    def __init__(
        ...
        if not self.local_files_only:
            raise ModelConfigurationError(
                "FinBertPtBrModel exige local_files_only=true. "
    ...
    def _configuration_boolean(
        ...
        *,
        ...
        value = mapping[key]
        normalized = FinBertPtBrModel._optional_boolean(
            value,
            key,
        )
    ...
    def _parse_positive_integer(
        ...
        normalized_value = FinBertPtBrModel._parse_integer(
            value,
            field_name,
        )
